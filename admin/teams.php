      ...
      <section class="content">
        <div>
          ...
          if ($stmt->rowcount() <= 0) { ?>
            ...
            </div>Your Request Adviser has been APPROVED
          <?php } else {
        ...
        <?php if ($stmt->rowcount() > 0) { ?>
          ...
          <div class="row">
            <?php
            $sql = "SELECT *,teams_member.users_id AS teams_member_users_id FROM teams_member INNER JOIN users ON teams_member.users_id=users.id WHERE teams_member.teams_id='".$row4['teams_teams_id']."'";
            $stmt = $this->conn()->query($sql);
            ...
            ?>
                <div class="col-lg-2 col-12">
                  <div data-target="#addmember" data-toggle="modal" class="small-box" style="box-shadow: 5px 5px 10px rgba(0, 0, 0, 0.3); background-color: #e7e9ec6e !important;color:#fff;border-radius: 30px;padding: 30px 0px;cursor: pointer;height: 350px;display: grid;place-items: center;">
                    <div class="inner" style="text-align: center;">
                        <i class="fas fa-plus" style="font-size: 35px;color:#000;"></i>
                      ...
                      <tr>
                        ...
                        <td style="font-size: 23px;font-style: normal;">
                          ...
                          <?php if($_SESSION['type'] == 1): ?>
                            ...
                            >Change Group Status</button>
                              <button class='btn btn-danger btn-sm deleteteams btn-table' 
                              >REMOVE</button>

                            <?php else: ?>
                              <button class='btn btn-primary btn-sm changestatus2 btn-table' 
                              data-changestatus2_teams_id='<?php echo $row['teams_id'] ?>'
                              >EDIT</button>
                            <?php endif; ?>
                            <?php if($row['adviser_status'] == 1): ?>
